...
namespace impl
{
    ...
    namespace ports
    {
        ...
        const std::vector<port> all{ boost::copy_range<std::vector<port>>(boost::range::join(boost::range::join(rtp, ws), usb)) };
    }
...
nmos::connection_resource_auto_resolver make_node_implementation_auto_resolver(const nmos::settings& settings)
{
    ...
    const auto usb_sender_ports = boost::copy_range<std::vector<impl::port>>(impl::parse_ports(impl::fields::senders(settings)) | boost::adaptors::filtered(impl::is_usb_port));
    ...
    const auto usb_receiver_ports = boost::copy_range<std::vector<impl::port>>(impl::parse_ports(impl::fields::receivers(settings)) | boost::adaptors::filtered(impl::is_usb_port));
    ...
    return [rtp_sender_ids, rtp_receiver_ids, ws_sender_ids, ws_sender_uri, ws_receiver_ids, usb_sender_ids, usb_receiver_ids](const nmos::resource& resource, const nmos::resource& connection_resource, value& transport_params)
    {
...
nmos::connection_sender_transportfile_setter make_node_implementation_transportfile_setter(const nmos::resources& node_resources, const nmos::settings& settings)
{
    ...
    {
        //TODO: Just to prove it works, it is necessary to
        // build USB SDP 
